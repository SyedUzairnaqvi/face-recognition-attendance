...
def build_embedding_index(known_faces_dir: Path) -> dict:
    ...
    files = sorted(p for p in known_faces_dir.rglob("*") if p.suffix.lower() in image_extensions)
...
def load_embedding_index() -> Dict[str, np.ndarray]:
    if not EMBEDDING_INDEX_PATH.exists():
        raise FileNotFoundError(
            f"Embedding index not found at {EMBEDDING_INDEX_PATH}. Run scripts/build_embeddings.py first."
        )

    data = np.load(EMBEDDING_INDEX_PATH, allow_pickle=False)
    model = str(data["model"][0])
    if model != EMBEDDING_MODEL_NAME:
        raise RuntimeError(f"Index uses {model}, but application is configured for {EMBEDDING_MODEL_NAME}.")
